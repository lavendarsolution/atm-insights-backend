...
class PredictionScheduler:
    ...
    async def start_prediction_scheduler(self):
        ...
        self.prediction_task = asyncio.create_task(self._prediction_loop(interval=1200))  # 20 minutes
    ...
    async def _run_bulk_predictions(self):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"✅ Completed predictions for {len(predictions)} ATMs in {processing_time:.2f}s")
    ...
    async def _store_prediction_summary(self, predictions):
        ...
        try:
                ...
                "high_risk_atms_list": [
                    {
                        ...
                        "top_risk_factors": p.top_risk_factors[:3] if p.top_risk_factors else []  # Top 3 factors
                    }
                ...
                "critical_risk_atms_list": [
                    {
                        ...
                        "confidence": p.confidence,
                        "risk_level": p.risk_level,
                        "top_risk_factors": p.top_risk_factors[:3] if p.top_risk_factors else []
                    }
                    for p in critical_risk_atms[:10]  # Limit to top 10 critical-risk ATMs
                ]
            }
            ...
            await self.cache_service.set("ml_high_risk_atms", high_risk_atms[:20], ttl=1800)
            await self.cache_service.set("ml_critical_risk_atms", critical_risk_atms[:10], ttl=1800)
            
            logger.info(f"📊 Stored prediction summary: {len(high_risk_atms)} high-risk, {len(critical_risk_atms)} critical-risk ATMs")
    ...
    async def _check_high_risk_atms(self, predictions):
        ...
        try:
            ...
            critical_risk_atms = [p for p in predictions if p.failure_probability > critical_threshold]
            high_risk_atms = [p for p in predictions if high_threshold < p.failure_probability <= critical_threshold]
            ...
            if critical_risk_atms:
                ...
                logger.warning(f"🚨 Published CRITICAL risk alert for {len(critical_risk_atms)} ATMs")
            ...
            if len(high_risk_atms) >= 5:  # Only alert if 5+ ATMs are high risk
                ...
                logger.info(f"⚠️ Published HIGH risk alert for {len(high_risk_atms)} ATMs")

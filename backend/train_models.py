...
def run_benchmarks():
    ...
    try:
        ...
        # Get sample ATM IDs
        sample_atms = db.execute(text(
            "SELECT DISTINCT atm_id FROM atm_telemetry LIMIT 50"
        )).fetchall()
...
def main():
    ...
    try:
        ...
        print(f"   Training samples: {results.get('positive_samples', 0) + results.get('negative_samples', 0):,}")
        ...
        # Performance summary
        total_records = results.get('positive_samples', 0) + results.get('negative_samples', 0)
        if total_records > 0:
            print(f"\n📋 TRAINING SUMMARY:")
            ...
            print(f"   ✅ Production-ready model with AUC: {results.get('auc_score', 0):.4f}")

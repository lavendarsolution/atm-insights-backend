...
from config import SIMULATOR_CONFIG, REGIONS, ERROR_CODES, generate_atm_config
...
class ATMSimulator:
    ...
    def _generate_realistic_telemetry(self, atm: Dict) -> Dict:
        ...
        temperature = base_temp + random.uniform(-2, 3) + (1 - health) * random.uniform(0, 8)
        
        # Cash level simulation
        days_since_maintenance = (now - atm["last_maintenance"]).days
        daily_usage = random.uniform(5, 15)  # % per day
        cash_level = max(5, 100 - (days_since_maintenance * daily_usage) + random.uniform(-10, 5))
        ...
        transactions = int(random.poisson(8 * transaction_multiplier * health))
    ...
    async def _send_telemetry_batch(self, session: aiohttp.ClientSession, atm_batch: List[Dict]):
    ...
    async def _send_single_telemetry(self, session: aiohttp.ClientSession, telemetry: Dict) -> bool:
    ...
    async def _simulation_cycle(self):
        ...
        logger.info(f"📊 Cycle completed: {len(self.atms)} ATMs in {duration:.2f}s | Success rate: {success_rate:.1f}%")
